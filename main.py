...
def answer_question(user):
    if len(user.questions) > 0:
        ...
        print_root()
        q.add_answer(Answer(user, True if get_height() > 3 else random.randint(0,1) == 1))
...
def sibling_consensus(question):
    ...
    if len(sibs) > 1 and reduce(lambda a, q : not q.reasked and q.check_consensus() and a, sibs, True):
        parent = question.parent
        if not parent.closed:
            print("the reask %s" % parent.name)
            input()
            parent.closed = True
...
def close_answers():
    ...
    for question in questions:
        if not question.closed and not question.check_consensus():
            print("no consensus")
            print_root()
